...
| Session persistence and logout     | Short-lived access JWTs, opaque rotating refresh tokens with family max age and device bind, `/auth/refresh`, and `/auth/logout` (family revoke or `allDevices`) | Dual-token SecureStore persistence, silent refresh+retry on 401, best-effort logout revoke, and account “sign out of all devices” | API unit/e2e session specs; mobile token-store, API interceptor, AuthProvider dual-token and all-devices logout tests; live mobile API-client bearer restore/logout behavior               | Soften expiry UX messaging on native; keep rotation/revocation incident response runbooks current                                                                                         |
...
3. Soften production session expiry UX on native; keep rotation/revocation
   ...
   and customer “sign out of all devices” are implemented).
