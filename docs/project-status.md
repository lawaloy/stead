...
| Session persistence and logout     | Short-lived access JWTs, opaque rotating refresh tokens, `/auth/refresh`, and `/auth/logout` revoke                                                         | Dual-token SecureStore persistence, silent refresh+retry on 401, and best-effort logout revoke                 | API unit/e2e session specs; mobile token-store, API interceptor, and AuthProvider dual-token tests; live mobile API-client bearer restore/logout behavior                                  | Soften expiry UX messaging on native; add full route-level journey coverage                                                                                                               |
...
3. Soften production session expiry UX on native and keep rotation/revocation
