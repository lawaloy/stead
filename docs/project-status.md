...
| Session persistence and logout     | Short-lived access JWTs, opaque rotating refresh tokens with family max age, device bind, and max 5 concurrent families; `/auth/refresh` and `/auth/logout` (family revoke or `allDevices`) | Dual-token SecureStore persistence, silent refresh+retry on 401, best-effort logout revoke, and account “sign out of all devices” | API unit/e2e session specs; mobile token-store, API interceptor, AuthProvider dual-token and all-devices logout tests; live mobile API-client bearer restore/logout behavior               | Keep rotation/revocation incident response runbooks current; tune `AUTH_REFRESH_MAX_FAMILIES_PER_USER` if product needs change                                                            |
...
3. Soften production session expiry UX on native; keep rotation/revocation
   incident response runbooks current (family max age, device-bound refresh,
   concurrent family cap, and customer “sign out of all devices” are
   implemented).
