...
describe('account screen delete cancel', () => {
  beforeEach(() => {
    queryClient.clear();
    jest.clearAllMocks();
  ...
  it('keeps the session when the customer cancels permanent deletion', async () => {
    await renderScreen();
    ...
    expect(actions?.some((action) => action.text === 'Keep account')).toBe(
      true,
    );
    await act(async () => {
      actions?.find((action) => action.text === 'Keep account')?.onPress?.();
    });
