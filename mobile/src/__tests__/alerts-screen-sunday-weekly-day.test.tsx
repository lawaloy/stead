...
describe('alerts screen Sunday weekly day', () => {
  beforeEach(() => {
    queryClient.clear();
    jest.clearAllMocks();
  ...
  it('saves Sunday as weeklyDay 0 so the API schedule matches the Sun control', async () => {
    await renderScreen();
    expect(await screen.findByRole('radio', { name: 'Mon' })).toBeSelected();
    expect(screen.getByRole('radio', { name: 'Sun' })).not.toBeSelected();

    await fireEvent.press(screen.getByRole('radio', { name: 'Sun' }));
    expect(screen.getByRole('radio', { name: 'Sun' })).toBeSelected();
