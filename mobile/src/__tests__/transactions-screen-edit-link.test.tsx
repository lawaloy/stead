...
describe('transactions screen edit link', () => {
  beforeEach(() => {
    queryClient.clear();
    jest.clearAllMocks();
    mockListTransactions.mockResolvedValue([unlinked]);
    ...
    mockUpdateTransaction.mockResolvedValue({ ...unlinked, goalId: 'goal_1' });
    jest.mocked(deleteTransaction).mockResolvedValue({ ok: true });
  });

  it('sends the active goal id when the customer links an untagged transaction', async () => {
    await renderScreen();
    ...
    await fireEvent.press(
      screen.getByRole('checkbox', { name: 'Count toward Emergency fund' }),
    );
