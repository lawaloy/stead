...
describe('transactions screen edit unlink', () => {
  beforeEach(() => {
    queryClient.clear();
    jest.clearAllMocks();
    ...
    mockUpdateTransaction.mockResolvedValue({ ...rows[0], goalId: null });
    jest.mocked(deleteTransaction).mockResolvedValue({ ok: true });
  });

  it('sends goalId null when the customer unchecks the current goal link', async () => {
    await renderScreen();
    ...
    await fireEvent.press(
      screen.getByRole('checkbox', { name: 'Keep the current goal link' }),
    );
