...
describe('goal screen end cancel', () => {
  beforeEach(() => {
    queryClient.clear();
    jest.clearAllMocks();
  ...
  it('keeps the active goal when mark-completed confirmation is cancelled', async () => {
    await renderScreen();
    ...
    expect(actions?.some((action) => action.text === 'Keep goal')).toBe(true);
    await act(async () => {
      actions?.find((action) => action.text === 'Keep goal')?.onPress?.();
    });
  ...
  it('keeps the active goal when cancel-goal confirmation is cancelled', async () => {
    await renderScreen();
    expect(await screen.findByText('Annual rent')).toBeOnTheScreen();
