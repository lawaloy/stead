...
Warm Android runs rely on cached AVD boot snapshots; both platforms cache
installable binaries, Gradle/iOS build deps, and Metro transforms. Shared
non-secret CI toolchain versions are kept in `.github/ci.env`; update that file
when changing Node, npm, the Android API level, or the macOS runner. New pushes
