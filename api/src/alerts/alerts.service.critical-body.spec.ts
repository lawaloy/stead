...
describe('AlertsService warning → critical risk body', () => {
  ...
  it('publishes a critical risk SMS after a notified warning deteriorates', async () => {
    ...
    dashboard.getStability.mockResolvedValue({
      ...
      metrics: {
        ...
        status: 'critical',
        stabilityScore: 22,
        paceRequiredMonthlyKobo: 1_200_000,
      },
    });
    ...
    expect(notifications.publishReadinessAlert).toHaveBeenCalledWith({
      ...
      payload: {
        phone: '+2348012345678',
        body: 'Stead alert: Rent moved to critical (score 22). Review recent spending and your required savings pace.',
      },
    });
    ...
    >;
    expect(upsertCalls[0]?.[0].update.lastNotifiedStatus).toBe('critical');
    expect(upsertCalls[0]?.[0].update.lastNotifiedScore).toBe(22);
