...
describe('AuthService request OTP create failure', () => {
  ...
  it('does not publish an SMS when the OTP row cannot be stored', async () => {
    ...
    prisma.otpCode.create.mockRejectedValue(
      new Error('otp create unavailable'),
    );
    ...
    ).rejects.toThrow('otp create unavailable');
