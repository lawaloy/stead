...
describe('AuthService verify OTP update failure', () => {
  ...
  it('does not issue a session or record verify telemetry when the attempt write fails', async () => {
    ...
    prisma.otpCode.update.mockRejectedValue(
      new Error('otp update unavailable'),
    );

    await expect(
      service.verifyOtp('08012345678', 'NG', '654321'),
    ).rejects.toThrow('otp update unavailable');
